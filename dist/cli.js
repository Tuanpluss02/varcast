...
function findCycle(startId, allVars) {
  ...
  while (stack.length > 0) {
    const { id, path: path3 } = stack.pop();
    if (path3.includes(id)) {
      const start = path3.indexOf(id);
      return [...path3.slice(start), id];
    }
    const v = allVars.get(id);
    if (!v)
      continue;
    const nextPath = [...path3, id];
...
function emitCollection(col, varIndex) {
  ...
  const groupClass = (path3) => cn + path3.join("");
...
// src/generator/write_node.ts
var fs = __toESM(require("fs"));
var path = __toESM(require("path"));
function writePackage(files, outDir) {
...
function main() {
  ...
  const irPath = path2.resolve(args.ir);
  const outDir = path2.resolve(args.out);
  const ir = JSON.parse(fs2.readFileSync(irPath, "utf-8"));
  const result = validate(ir);
  if (result.errors.length > 0) {
    console.error("Validation errors \u2014 emit blocked:");
    for (const e of result.errors)
      console.error(`  ${e.type}: ${e.path.join(" \u2192 ")}`);
    process.exit(1);
  }
